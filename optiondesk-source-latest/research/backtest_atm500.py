"""
OptionDesk Backtest ATM LEAPS ~500d Variant — 8 June 2025
==========================================================
ATM VARIANT: Strike = spot price (K = S0, OTM_PCT = 0.0)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(CACHE_DIR / "backtest_atm500_run.log"),
...
def run_backtest(sample_size: int | None = None):
    ...
    out_path = Path("/home/user/workspace/optiondesk/research/backtest_atm500_results.json")
    ...
    print("\n" + "="*60)
    print("BACKTEST SUMMARY (ATM LEAPS ~500d VARIANT) — OptionDesk Recommendations 2025-06-08")
    print("="*60)
    print(f"Universe: {total} tickers  |  Scored: {len(scored)}  |  Failed: {len(failed)}")
    print(f"Top-20 analysis | Entry date: {ENTRY_DATE} | ~{TARGET_DTE_DAYS}d DTE (LEAPS) | ATM calls (OTM_PCT={OTM_PCT})")
    print()
