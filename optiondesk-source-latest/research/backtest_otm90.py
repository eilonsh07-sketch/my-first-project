"""
OptionDesk Backtest SHORT-DTE 90-Day OTM Variant — 8 June 2025
===============================================================
SHORT-DTE OTM VARIANT: 90-day (~3-month) expiry, 10% OTM strike (K = S0*1.10, OTM_PCT = 0.10)
All other methodology identical to the ATM 90-day backtest.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(CACHE_DIR / "backtest_otm90_run.log"),
...
def run_backtest(sample_size: int | None = None):
    ...
    output = {
        "meta": {
            ...
            "otm_pct": OTM_PCT,
            "strike_type": "OTM",
            "variant": "SHORT_DTE_90d_OTM_10pct",
            "risk_free": RISK_FREE,
    ...
    out_path = Path("/home/user/workspace/optiondesk/research/backtest_otm90_results.json")
    ...
    print("\n" + "="*60)
    print("BACKTEST SUMMARY (SHORT-DTE 90d OTM 10% VARIANT) — OptionDesk Recommendations 2025-06-08")
    print("="*60)
    print(f"Universe: {total} tickers  |  Scored: {len(scored)}  |  Failed: {len(failed)}")
    print(f"Top-20 analysis | Entry date: {ENTRY_DATE} | ~{TARGET_DTE_DAYS}d DTE | 10% OTM calls (OTM_PCT={OTM_PCT})")
