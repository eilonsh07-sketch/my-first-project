"""
OptionDesk Backtest ITM-20 LEAPS ~500d Variant — 8 June 2025
=============================================================
ITM-20 VARIANT: Strike = 80% of spot price (K = 0.80 * S0, OTM_PCT = -0.20)
...
OTM_PCT     = -0.20                # 20% ITM strike (K = 0.80 * S0)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(CACHE_DIR / "backtest_itm20_500_run.log"),
...
def run_backtest(sample_size: int | None = None):
    # ── ITM-20 STRIKE DIRECTION VERIFICATION ────────────────────────────────
    print("\n=== ITM-20 STRIKE DIRECTION VERIFICATION ===")
    print(f"OTM_PCT = {OTM_PCT}  (negative = ITM, K < S0)")
    sample_prices = [100.0, 250.0, 500.0, 1000.0]
    for sp in sample_prices:
        K_test = round(sp * (1.0 + OTM_PCT), 2)
        print(f"  S0={sp:.2f}  K={K_test:.2f}  K/S0={K_test/sp:.3f}  K<S0: {K_test < sp}")
    assert OTM_PCT == -0.20, f"OTM_PCT must be -0.20 for ITM-20, got {OTM_PCT}"
    assert all(round(sp * (1 + OTM_PCT), 2) < sp for sp in sample_prices), "K must be < S0 for ITM"
    print("VERIFICATION PASSED: K = 0.80 * S0 < S0 (20% ITM confirmed)")
    print("============================================\n")
    ...
    out_path = Path("/home/user/workspace/optiondesk/research/backtest_itm20_500_results.json")
    ...
    print("\n" + "="*60)
    print("BACKTEST SUMMARY (ITM-20 LEAPS ~500d VARIANT) — OptionDesk Recommendations 2025-06-08")
    print("="*60)
    print(f"Universe: {total} tickers  |  Scored: {len(scored)}  |  Failed: {len(failed)}")
    print(f"Top-20 analysis | Entry date: {ENTRY_DATE} | ~{TARGET_DTE_DAYS}d DTE (LEAPS) | ITM-20 calls (OTM_PCT={OTM_PCT}, K=0.80*S0)")
    print()
