"""
OptionDesk Backtest SHORT-DTE 90-Day Variant — 8 June 2025
===========================================================
SHORT-DTE VARIANT: 90-day (~3-month) expiry, ATM strike (K = S0, OTM_PCT = 0.0)
All other methodology identical to the original ATM 270-day backtest.
...
SETTLEMENT RULE FOR SHORT-EXPIRY OPTIONS:
  - The option expires ~90 days after entry (~2025-09-06).
  - 3-month horizon (~2025-09-08): approximately at expiry. If T_remaining > 0,
    reprice via B-S; if expired, use intrinsic max(S_expiry - K, 0).
  - 6-month horizon (2025-12-08): PAST expiry. Settle at expiry (intrinsic) and
    hold cash. Return = intrinsic/premium - 1 (no time value, no reprice).
  - 12-month horizon (2026-06-08): PAST expiry. Same rule - settle at expiry.
  This avoids repricing a "dead" option and preserves leak-free integrity.
...
TARGET_DTE_DAYS = 90               # ~3-month expiry (short DTE variant)
OTM_PCT     = 0.00                 # 0% OTM strike (ATM)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(CACHE_DIR / "backtest_dte90_run.log"),
...
# ─── Exit pricing (DTE-90 settlement logic) ────────────────────────────────
def compute_exit_value(
    row: dict, exit_df: pd.DataFrame, horizon_date: date
) -> dict:
    """
    SHORT-DTE (90d) settlement rule:

    The option expires ~90 days after entry (~2025-09-06).

    - If horizon_date < expiry: option is still alive -> reprice via B-S
      (standard Approach A with actual S_exit + realized vol).

    - If horizon_date >= expiry: option has already expired.
      -> Settle AT EXPIRY: exit_value = intrinsic = max(S_expiry - K, 0).
         6mo/12mo horizons confirm the payout realised at expiry.
         No re-pricing of a dead option.
         stock_return is still measured to the ACTUAL horizon_date for
         comparability with SPY over the same window.
    """
    K = row["K"]
    ...
    if horizon_date >= expiry:
        # Option has expired: settle at intrinsic(S_expiry)
        S_exp = get_close_on_date(exit_df, expiry)
        S_horizon = get_close_on_date(exit_df, horizon_date)

        if S_exp is None:
            return {"S_exit": S_horizon, "exit_value": None, "option_return": None,
                    "stock_return": None, "horizon": str(horizon_date),
                    "settlement": "at_expiry", "S_expiry": None, "T_remaining": 0.0}

        intrinsic = max(S_exp - K, 0.0)
        exit_value = intrinsic
        option_return = (exit_value / entry_premium - 1.0) if entry_premium > 0 else None
        stock_return = (S_horizon / S0 - 1.0) if (S_horizon is not None and S0 > 0) else None
        stock_return_at_expiry = (S_exp / S0 - 1.0) if S0 > 0 else None
        ...
        return {
            ...
            "stock_return_at_expiry": stock_return_at_expiry,
            "settlement": "at_expiry_intrinsic",
            "settlement_note": (
                f"Option expired {expiry}. Intrinsic={intrinsic:.4f} "
                f"(S_expiry={S_exp:.2f}, K={K:.2f}). "
                "Horizon return locked to expiry payoff."
            ),
        }
    else:
        # Option still alive: reprice via B-S
...
def run_backtest(sample_size: int | None = None):
    ...
    _fmt = lambda v: f"{v:.1%}" if v is not None else "N/A"
    log.info(f"SPY S0={spy_s0:.2f}  3mo={_fmt(spy_ret['3mo'])}  6mo={_fmt(spy_ret['6mo'])}  12mo={_fmt(spy_ret['12mo'])}")
    ...
    # ── Step 6: Sanity check — NVDA & PLTR (DTE-90 expiry verification) ──
    log.info("\n=== SANITY CHECK (DTE-90: option expires ~2025-09-06) ===")
    log.info("  VERIFY: 6mo/12mo must show settlement=at_expiry_intrinsic; 3mo may show alive_bs_reprice.")
    for chk_ticker in ["NVDA", "PLTR", "AAPL"]:
        ...
        if not matches:
            ...
            continue
        r = matches[0]
        log.info(f"  {chk_ticker}: S0={r['S0']:.2f}  K={r['K']:.2f}  expiry={r['expiry']}  "
                 f"premium={r['entry_premium']:.2f}  score={r['combined_score']:.1f}")
        for h_name, h_dt in horizons:
            ex = r["exits"].get(h_name, {})
            s_exit = ex.get("S_exit")
            s_expiry_val = ex.get("S_expiry")
            o_ret = ex.get("option_return")
            st_ret = ex.get("stock_return")
            settlement = ex.get("settlement", "unknown")
            t_rem = ex.get("T_remaining")
            if o_ret is not None:
                s_expiry_str = f"  S_expiry={s_expiry_val:.2f}" if s_expiry_val is not None else ""
                st_ret_str = f"{st_ret:.1%}" if st_ret is not None else "N/A"
                log.info(
                    f"    {h_name} ({h_dt}): settlement={settlement}  "
                    f"S_exit={s_exit}{s_expiry_str}  T_rem={t_rem}  "
                    f"option_ret={o_ret:.1%}  stock_ret={st_ret_str}"
                )
            else:
                log.info(f"    {h_name}: N/A")
    ...
    output = {
        "meta": {
            ...
            "otm_pct": OTM_PCT,
            "strike_type": "ATM",
            "variant": "SHORT_DTE_90d_ATM_0pct_OTM",
            "risk_free": RISK_FREE,
            ...
            "exit_method": "short_dte_settlement: alive=bs_reprice; past_expiry=intrinsic_at_expiry",
            "settlement_rule": (
                "Option expiry ~90d from entry (~2025-09-06). "
                "3mo horizon: reprice if alive, else intrinsic. "
                "6mo/12mo horizons: PAST expiry - settled at intrinsic(S_expiry). "
                "No re-pricing of dead options. Stock return measured to horizon_date."
            ),
            "leak_free": True,
            "caveats": [
                ...
                "6mo and 12mo option returns are identical to 3mo (all settled at same expiry intrinsic value)",
            ],
            "rng_seed": RNG_SEED,
        },
        "spy_benchmarks": {
    ...
    out_path = Path("/home/user/workspace/optiondesk/research/backtest_dte90_results.json")
    ...
    print("\n" + "="*60)
    print("BACKTEST SUMMARY (SHORT-DTE 90d VARIANT) — OptionDesk Recommendations 2025-06-08")
    print("="*60)
    print(f"Universe: {total} tickers  |  Scored: {len(scored)}  |  Failed: {len(failed)}")
    print(f"Top-20 analysis | Entry date: {ENTRY_DATE} | ~{TARGET_DTE_DAYS}d DTE | ATM calls (OTM_PCT={OTM_PCT})")
    print(f"Settlement: 6mo/12mo horizons settled at expiry intrinsic (option already expired).")
    print()
