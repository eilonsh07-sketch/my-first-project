"""
OptionDesk Backtest ITM-10 LEAPS ~500d Variant — 8 June 2025
=============================================================
ITM-10 VARIANT: Strike = 90% of spot (K = 0.90 * S0, OTM_PCT = -0.10)
LEAPS VARIANT: TARGET_DTE_DAYS = 500 (~500-day expiry)

Key LEAPS note:
  A 500-day option entered 2025-06-08 expires ~2026-10-20, which is AFTER
  our data window ends (2026-06-08). Therefore at ALL horizons (3/6/12mo)
  the option is STILL ALIVE — we NEVER hold to expiry. We evaluate by
  repricing the SAME contract with the actual close on the horizon date +
  realized vol of the window ending that date, with remaining time to expiry.
  This is Approach A ("sold before expiry by default") for all horizons.
...
TARGET_DTE_DAYS = 500              # ~500-day LEAPS expiry (~Oct 2026, after data window)
OTM_PCT     = -0.10                # 10% ITM strike (K = 0.90 * S0)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(CACHE_DIR / "backtest_itm10_500_run.log"),
...
def run_backtest(sample_size: int | None = None):
    ...
    # ── ITM-10 Strike Direction Verification ────────────────────────────────
    # Print a few (S0, K) pairs to confirm K < S0 for ITM call
    log.info("ITM-10 STRIKE DIRECTION CHECK: OTM_PCT=%.2f => K = S0 * (1 + OTM_PCT) = 0.90 * S0", OTM_PCT)
    log.info("Sampling a few tickers to confirm K < S0 (ITM call):")
    sample_check_tickers = tickers[:5]
    for stk in sample_check_tickers:
        df_chk = yf_download(stk, ENTRY_START, ENTRY_END)
        if df_chk is not None and not df_chk.empty:
            s0_chk = get_entry_price(df_chk)
            if s0_chk and s0_chk > MIN_PRICE:
                k_chk = round(s0_chk * (1.0 + OTM_PCT), 2)
                direction = "ITM-CALL-OK (K<S0)" if k_chk < s0_chk else "ERROR: K>=S0"
                log.info(f"  {stk}: S0={s0_chk:.2f}  K={k_chk:.2f}  {direction}")
    log.info("Strike direction check done.")
    ...
    output = {
        "meta": {
            ...
            "approach_b": "never_triggered_for_leaps_expiry_beyond_data_window",
            "leaps_still_alive": True,
            "leaps_expiry_approx": "2026-10-20",
            "data_window_end": "2026-06-08",
            "leak_free": True,
            "caveats": [
                ...
                "LEAPS STILL-ALIVE: all 3/6/12mo evaluations use Approach A BS-reprice (T_remaining > 0 at all horizons)",
                "Option expiry ~2026-10-20 is beyond data window end 2026-06-08; no hold-to-expiry for any horizon",
                "ITM-10: K = 0.90 * S0 (10% in-the-money), high delta, lower leverage vs ATM/OTM, higher hit-rate expected",
            ],
            "rng_seed": RNG_SEED,
        },
        "spy_benchmarks": {
    ...
    out_path = Path("/home/user/workspace/optiondesk/research/backtest_itm10_500_results.json")
    ...
    print("\n" + "="*60)
    print("BACKTEST SUMMARY (ITM-10 LEAPS ~500d VARIANT) — OptionDesk Recommendations 2025-06-08")
    print("="*60)
    print(f"Universe: {total} tickers  |  Scored: {len(scored)}  |  Failed: {len(failed)}")
    print(f"Top-20 analysis | Entry date: {ENTRY_DATE} | ~{TARGET_DTE_DAYS}d DTE (LEAPS) | ITM-10 calls (OTM_PCT={OTM_PCT}, K=0.90*S0)")
    print()
